...
- `/game-plugin:game-analysis` 스킬의 워크플로우를 그대로 따른다.
...
### Step 3: PRD 생성 (game-prd 스킬 실행)

Step 2에서 생성된 분석 보고서를 인풋으로 `game-prd` 스킬을 실행한다.

- `/game-plugin:game-prd` 스킬의 워크플로우를 그대로 따른다.
...
📊 분석 보고서: Docs/{게임명}_analysis.md
📋 PRD:         Docs/{게임명}_PRD.md

💡 구현을 시작하려면 /implement 를 입력하세요.
   PRD의 기능 목록에서 다음 항목을 자동으로 가져와 개발을 진행합니다.
```
