...
**B. 인자 있음**
- `/game-plugin:game-feature` 스킬의 워크플로우를 실행한다.
...
### Step 5: 세부 계획 수립 (game-plan 스킬 실행)

`game-plan` 스킬의 워크플로우 전체를 실행한다.

- `/game-plugin:game-plan` 스킬의 워크플로우를 그대로 따른다.
- 인풋: Step 4에서 확정된 기능명 및 설명
- 출력: `Docs/Features/{FeatureName}.md` 작업 명세서

game-plan 스킬은 계획 수립 후 사용자 승인을 받는다. 승인 없이 Step 6으로 넘어가지 않는다.
...
**실행 원칙**:
- 코드 작업은 `/game-plugin:game-tdd` 스킬의 워크플로우를 따른다. TDD 사이클(Red → Green → Refactor)을 적용한다.
...
- `/game-plugin:game-feature` 스킬의 "완료 표시" 절차를 따른다.
- 해당 항목: `[ ]` → `[x]`, 날짜 갱신.
...
`game-commit` 스킬을 실행하여 커밋 메시지를 작성하고 커밋한다.

- `/game-plugin:game-commit` 스킬의 절차를 따른다.
- 이 단계에서는 사용자 확인을 받는다.
...
- 코드 작업 시 game-tdd 스킬의 TDD 사이클을 반드시 준수한다.
