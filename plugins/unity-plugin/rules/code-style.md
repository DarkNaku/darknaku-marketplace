...
```csharp
// ✅ DO: 회사.프로젝트.기능 계층 구조 (PascalCase, 특수문자·밑줄 금지)
...
- 네임스페이스는 **폴더 구조를 반영**하는 것을 권장 (관련 클래스를 논리적으로 그룹화)
...
{
    // ── 상수: k_PascalCase ───────────────────────────────
    private const float k_GravityScale = 9.81f;
    ...
    // ── 이벤트: 동사구, "On" 접두어 없이 ─────────────────
    // Before 이벤트: 현재분사  /  After 이벤트: 과거분사
    ...
    private void OnDamageTaken(float amount) => DamageTaken?.Invoke(amount);
...
- **명사를 사용한다**: 변수는 상태나 사물을 나타내므로 명사로 짓는다.  
- **Boolean은 동사 접두어를 붙인다**: `_isDead`, `_isGrounded`, `_hasDamageMultiplier`  
- **bool을 반환하는 메서드는 질문 형식으로 짓는다**: `IsGameOver()`, `HasStartedTurn()`  
- **축약어를 쓰지 않는다** (루프 카운터 `i, j, k` 및 공인 약어 `ID, UI, AI, HP, MP` 제외)  
- **중복 이름을 피한다**: 클래스가 `Player`이면 멤버는 `PlayerScore`가 아닌 `Score`로  
- **변수 하나에 선언 한 줄**: 가독성을 위해 여러 변수를 한 줄에 선언하지 않는다

```csharp
// ❌ 한 줄에 여러 변수 선언
private int _health, _mana, _stamina;
...
```csharp
// ── 이벤트 자체: 동사구 (시제로 Before / After 구분) ──────
public event Action OpeningDoor;   // Before — 현재분사
public event Action DoorOpened;    // After  — 과거분사

public event Action<float> DamageTaken;
public event Action        Death;

// ── 발행 메서드 (Subject 쪽): "On" + 이벤트명 ──────────────
private void OnDoorOpened()       => DoorOpened?.Invoke();
private void OnDamageTaken(float amount) => DamageTaken?.Invoke(amount);

// ── 구독 핸들러 (Observer 쪽): "발행자명_이벤트명" ──────────
private void GameEvents_DoorOpened()     { ... }
private void EnemyController_DamageTaken(float amount) { ... }
```

### 3-5. 열거형 규칙

```csharp
// ✅ 열거형 이름은 단수 명사 + PascalCase
...
// ❌ 열거형 값에 열거형 이름을 반복
public enum Direction { DirectionNorth, DirectionSouth }  // 금지
...
// ❌ 상수에 UPPER_SNAKE_CASE 사용
private const int k_MAX_SPEED = 10;  // 금지

// ✅ 상수는 k_PascalCase
private const int k_MaxSpeed = 10;
```
...
// ✅ 연산자 주변 공백
int total = a + b * c;
bool isValid = count > 0 && count < k_MaxPoolSize;
...
```csharp
// ✅ 읽기 전용 단일 라인 → expression-bodied (=>) 사용
public bool IsAlive   => _currentHealth > 0;
public int  MaxHealth => _maxHealth;

// ✅ get/set이 모두 필요한 경우 → 표현식 바디 accessor + setter는 private
...
// ✅ backing field가 필요 없는 자동 프로퍼티
public bool IsInvincible { get; private set; }

// ❌ 단순 read-only에 불필요한 전체 블록
public int MaxHealth
{
    get { return _maxHealth; }  // 금지 — => 를 사용할 것
}
```

### 4-6. `var` 사용 규칙

타입이 **우측에서 명확히 드러나는 경우**에만 `var`를 사용한다.
...
// ❌ 반환 타입이 불명확한 경우 — 금지
var result = GetData();          // 어떤 타입인지 알 수 없음
var x      = ProcessEnemies();   // 금지
```
...
{
    // 1. 상수
    private const int k_MaxCount = 10;
    ...
    private void Initialize()       { s_totalCreated++; OnInitialized(); }
...
// ✅ [Header], [Tooltip] — Inspector 가독성 향상
[Header("Movement")]
...
private void Awake()
{
    _animRunHash = Animator.StringToHash("Run");
}
...
// ❌ 상수에 UPPER_SNAKE_CASE
private const int k_MAX_LIVES = 3;     // 금지
...
namespace AcmeCorp.ProjectName.Gameplay
{
    ...
    public sealed class PlayerHealth : MonoBehaviour, IDamageable
    {
        ...
        // ── SerializeField ────────────────────────────────
        [SerializeField, Range(1, 999)] private int   _maxHealth = 100;
        [SerializeField, Range(0f, 50f)] private float _regenRate = 5f;
        ...
        private static readonly WaitForSeconds s_regenWait =
            new WaitForSeconds(k_RegenInterval);
