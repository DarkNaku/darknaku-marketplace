---
name: unity-tdd
description: Unity 프로젝트에서 기능을 구현하거나 버그를 수정 및 디버깅할 때 사용합니다. 사용자가 Unity에서 기능 추가, 구현, 개발, 작업, 수정, 고치기, 디버깅, 버그 수정 등을 요청하면 반드시 이 스킬을 사용합니다.
...
Unity 테스트 작성 방법의 상세 내용은 → [unity-test-reference.md](unity-test-reference.md) 참조
...
테스트 어셈블리 구성, 파일/클래스 구성 규칙, 어트리뷰트 사용법, 코드 예시 → [unity-test-reference.md](unity-test-reference.md) 참조
...
테스트 메서드명은 **한국어 + 언더스코어**로 동작을 서술한다.

```csharp
[Test] public void 항목이_없을때_비어있음을_반환한다() { }
[Test] public void 항목을_추가하면_개수가_증가한다() { }
[Test] public void 존재하지_않는_항목을_제거하면_예외가_발생한다() { }
```
...
- 테스트를 실행(Step 4와 동일한 방식)해 Green을 확인한다
