...
- 출력: `/mnt/user-data/outputs/{게임명}_GRD.md`
...
- dk-analysis → dk-grd 순서는 반드시 지킨다. GRD는 분석 보고서 없이 생성할 수 없다.
- 각 스킬의 품질 기준을 모두 충족한 후 다음 단계로 넘어간다.
- 분석 보고서의 정보가 부족한 경우에도 추정값을 포함해 GRD를 완성한다.
