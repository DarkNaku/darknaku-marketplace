...
description: 현재 프로젝트를 직접 탐색하여 패키지·라이브러리, 아키텍처 레이어, 코딩 스타일·네이밍 규칙, 핵심 패턴, 폴더 구조, 테스트 구조, 개발 환경을 추출하고 Claude Code가 참조할 수 있는 PROJECT_CONTEXT.md 문서로 저장하는 스킬. 사용자가 "프로젝트 분석해줘", "아키텍처 파악해줘", "코딩 컨벤션 뽑아줘", "프로젝트 파악", "컨텍스트 문서 만들어줘", "이 프로젝트 어떻게 구성돼 있어?" 등의 표현을 사용하거나, 새 프로젝트에 처음 진입할 때 반드시 이 스킬을 사용하라. Unity 프로젝트를 우선 지원하며 범용 프로젝트도 분석 가능하다.
---
...
핵심 소스 파일 샘플을 읽어 아키텍처 패턴을 추론한다.

```bash
# 대표 소스 파일 샘플링 (각 폴더에서 1~3개)
find Assets/Scripts -name "*.cs" | head -30 | xargs ls -la 2>/dev/null

# 레이어 폴더명으로 추론
find Assets/Scripts -maxdepth 3 -type d | sort

# 대표 파일 내용 읽기 (Domain, Application, Presentation 각 1개)
find Assets/Scripts -name "*.cs" -path "*/Domain/*" | head -3
find Assets/Scripts -name "*.cs" -path "*/Application/*" | head -3
find Assets/Scripts -name "*.cs" -path "*/Presentation/*" | head -3
find Assets/Scripts -name "*.cs" -path "*/Infrastructure/*" | head -3
```

읽은 파일에서 추론할 항목:
- 레이어 구분 방식 (네임스페이스 / 폴더 / 어셈블리)
- 의존성 방향 (어느 레이어가 어느 레이어를 참조하는가)
- DDD 요소 존재 여부 (Entity, ValueObject, Repository, UseCase 등)
- MVP/MVC/MVVM 중 어떤 패턴인가

### Step 6: 코딩 스타일 & 네이밍 규칙 추출

대표 파일 5~10개를 직접 읽어 패턴을 추출한다.
...
grep -r "VContainer\|IContainerBuilder\|RegisterEntryPoint\|Zenject\|Bind" \
  Assets/Scripts --include="*.cs" -l | head -10

# 이벤트/메시지 패턴
grep -r "MessagePipe\|IPublisher\|ISubscriber\|Subject\|Observable\|AddTo" \
  Assets/Scripts --include="*.cs" -l | head -10
...
# 리포지터리/유스케이스 패턴
grep -r "IRepository\|UseCase\|Interactor\|IUseCase" \
  Assets/Scripts --include="*.cs" -l | head -10

# 결과 타입 (Result<T>, Either 등)
grep -r "Result<\|IResult\|Either<\|Option<" \
  Assets/Scripts --include="*.cs" -l | head -5
```

발견된 패턴 파일을 1~2개 직접 읽어 **실제 사용 예시 코드**를 추출한다.

### Step 8: 테스트 구조 분석

```bash
# 테스트 파일 목록
find . -name "*.Tests.asmdef" -o -name "*Tests.cs" -o -name "*Test.cs" \
  | head -20
...
추출 항목: 테스트 프레임워크, Mock 라이브러리, Arrange/Act/Assert 패턴, 테스트 클래스 네이밍, 픽스처 사용 방식

### Step 9: PROJECT_CONTEXT.md 작성
...
- Claude Code가 `/new-feature` 실행 시 이 문서만 보고 구현 방향을 결정할 수 있는 수준

### Step 10: 저장 & 전달

```bash
# 프로젝트 루트에 저장 (Claude Code가 바로 참조 가능)
# 또는 사용자가 지정한 경로에 저장
```
...
| 패키지 감지 | 추론 패턴 |
|-------------|-----------|
| `com.annulusgames.lit-motion` 또는 `com.digitalruby.rainmaker` | 트위닝 패턴 |
| `com.cysharp.unitask` | 모든 비동기는 UniTask 사용 |
| `com.cysharp.r3` 또는 `com.neuecc.unirx` | 반응형 프로퍼티 / 스트림 패턴 |
| `com.hadashigames.vcontainer` 또는 `com.unity.vcontainer` | VContainer DI |
...
```bash
find Assets -name "*.asmdef" | xargs grep -l "name" 2>/dev/null
```
...
- [ ] 새 기능 추가 시 어느 폴더에 어떤 파일을 만들어야 하는지 명확함
