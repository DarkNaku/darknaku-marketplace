...
- 기능 개발·수정 또는 다른 버그 수정 진행 중에는 명시적 요청 시에만 발동

#### `/dk-test-validator`
테스트 코드가 올바른 이름과 내용을 갖추고 있는지, 실제로 필요한 테스트인지 검토하고 개선을 권장합니다.

- 클래스명이 테스트 내용을 포괄하는지, 메서드명이 조건·행위·기대값을 담고 있는지 확인
- 예약어가 아닌 이상 클래스명·메서드명에 "테스트"/"Test" 문구를 포함하지 않도록 권장
- 테스트명을 사용자 언어로 작성하도록 권장
